...
# Node List 服务器节点
PRE_LIST="192.168.1.5"        # 预生产节点
GROUP1_LIST="192.168.1.5 192.168.1.6  192.168.1.8"
GROUP2_LIST=" 192.168.1.6"
ROLLBACK_LIST="192.168.1.5 192.168.1.6  192.168.1.8"
...
group1_deploy(){ # 代码解压部署函数
    ...
    scp ${CONFIG_DIR}/other/192.168.1.5.server.xml 192.168.1.5:/webroot/web-demo/server.xml  # 将差异项目的配置文件scp到此web服务器并以项目结尾
}    
...
rollback(){ # 代码回滚主函数
    ...
    case $1 in # 把第二个参数做当自己的第一个参数     
        list)
            ls -l /opt/webroot/*.tar.gz        #执行脚本rollback 后，版本号不能加.tar.gz
            ;;
        *)
            rollback_fun $1
    esac
